...
NSString * const VideoDownloadErrorDomain = @"com.66yxq.download";
NSString * const VideoDownloadErrorHTTPStatusKey = @"VideoDownloadErrorHTTPStatusKey";
...
@property (nonatomic, assign, readwrite) VideoDownloadState state;
// Download
                   ...
                   delegate:(id<VideoDownloaderDelegate>)delegateOrNil
{
    ...
    if (self) {
        ...
        self.state = VideoDownloadStateReady;
...
- (void)start
{
    ...
    if (![NSURLConnection canHandleRequest:self.fileRequest]) {
        NSError *error = [NSError errorWithDomain:VideoDownloadErrorDomain
                                             code:VideoDownloadErrorInvalidURL
    ...
    if (self.connection && ![self isCancelled]) {
        ...
        self.state = VideoDownloadStateDownloading;
...
- (BOOL)isExecuting
{
    return self.state == VideoDownloadStateDownloading;
}

- (BOOL)isCancelled
{
    return self.state == VideoDownloadStateCancelled;
}

- (BOOL)isFinished
{
    return self.state == VideoDownloadStateCancelled || self.state == VideoDownloadStateDone || self.state == VideoDownloadStateFailed;
}
...
- (void)connection:(NSURLConnection*)connection didReceiveResponse:(NSURLResponse *)response
{
    ...
    if (httpResponse.statusCode >= 400) {
        error = [NSError errorWithDomain:VideoDownloadErrorDomain
                                    code:VideoDownloadErrorHTTPError
                                userInfo:@{ NSLocalizedDescriptionKey:[NSString stringWithFormat:@"Erroneous HTTP status code %ld (%@)",
                                                                       (long) httpResponse.statusCode,
                                                                       [NSHTTPURLResponse localizedStringForStatusCode:httpResponse.statusCode]],
                                            VideoDownloadErrorHTTPStatusKey: @(httpResponse.statusCode) }];
    }
    
    long long expected = @(self.expectedDataLength).longLongValue;
    if ([VideoDownloader freeDiskSpace].longLongValue < expected && expected != -1) {
        error = [NSError errorWithDomain:VideoDownloadErrorDomain
                                    code:VideoDownloadErrorNotEnoughFreeDiskSpace
...
- (void)cancel
{
    ...
    [self finishOperationWithState:VideoDownloadStateCancelled];
...
- (void)notifyFromCompletionWithError:(NSError *)error pathToFile:(NSString *)pathToFile
{
    ...
    VideoDownloadState finalState = success ? VideoDownloadStateDone : VideoDownloadStateFailed;
...
- (NSString *)fileName
{
    return _fileName ? _fileName : [[[NSURL URLWithString:[self.downloadURL absoluteString]] lastPathComponent] stringByAppendingString:@".mp4"];//mp4 文件
}
